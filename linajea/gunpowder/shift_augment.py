...
class ShiftAugment(BatchFilter):
    ...
    def __init__(
        ...
        Arguments:
            ...
            prob_slip (``float``):
                ...
                Probability that a single section will move independently
                of neighboring sections.

            prob_shift (``float``):

                Probability that a section and all following sections will
                shift.

            sigma (``float`` or ``float[]``):

                The standard deviation of shifting in each direction.
                If a single value, will be extended to all dimensions
                except the shift axis. If an array, must have length
                equal to the number of dimensions, and the value at
                shift_axis must be zero.

            shift_axis (``int``):

                The axis along which to shift. The algorithm will slice
                along this axis and shift sections relative to each other.
        '''
    ...
    def prepare(self, request):
        ...
        if not request.array_specs:
            raise ValueError(
                    "Request passed to ShiftAugment node must contain at least"
                    " one array key. Check to make sure that ShiftAugment"
                    " node is not upstream of a RandomLocation node "
        ...
        assert (roi_shape // self.lcm_voxel_size * self.lcm_voxel_size ==
                roi_shape), ("total roi shape {} must be divisible by least"
                             " common voxel size {}".format(
    ...
    def shift_and_crop(self, arr, roi_shape, sub_shift_array, voxel_size):
        """ Shift an array received from upstream and crop it to the target
            downstream region

        :param arr: an array of upstream data to be shifted and cropped
        :param roi_shape: the shape of the downstream ROI
        :param sub_shift_array: the cropped section of the global shift array
            that applies to this specific request
        ...
        assert array_shift_axis_len % sub_shift_array_len == 0, \
            ("array shift axis length {} is not divisible by "
             "the sub_shift_array length {}").format(
                arr.shape[self.shift_axis], sub_shift_array.shape[0])

        voxel_ratio = array_shift_axis_len // sub_shift_array_len

        # assumption: each sub shift array element divides evenly by voxel size
        rescaled_sub_shift_array = sub_shift_array // np.array(
                voxel_size, dtype=int)
        ...
        sl = tuple(slice(0, adjusted_roi_shape[index])
    ...
    def shift_points(
            ...
            shift_axis,
            lcm_voxel_size):
        """ Shift a set of points received from upstream and crop out those
            not the the target downstream region
        ...
        :param lcm_voxel_size: the least common voxel size for the arrays
            in the request
    ...
    def get_sub_shift_array(
            ...
            shift_axis,
            lcm_voxel_size):
        """ Slices the global shift array to return the sub-shift array
            to shift an item in the request
        ...
        :param lcm_voxel_size: the least common voxel size for the arrays
            in the request
        :return: the portion of the global shift array that should be used
            to shift the item
        """
        item_offset_from_total = item_roi.get_offset() - total_roi.get_offset()
        offset_in_shift_axis = (item_offset_from_total[shift_axis]
                                // lcm_voxel_size[shift_axis])
        len_in_shift_axis = (item_roi.get_shape()[shift_axis]
                             // lcm_voxel_size[shift_axis])
        return shift_array[offset_in_shift_axis:
                           offset_in_shift_axis + len_in_shift_axis]
    ...
    def construct_global_shift_array(
        ...
        :param shift_sigmas: the sigma to generate the normal distribution
            of shift amounts in each direction
        :param prob_slip: the probability of the slice shifting independently
            of all other slices
        :param prob_shift: the probability of the slice and all following
            slices shifting
        :param lcm_voxel_size: the least common voxel size of all the arrays
            in the request
        :return: the shift_array for the total_roi
        """

        assert shift_axis_len == 7,\
            "Request contains %d frames, expected 7" % shift_axis_len

        # each row is one slice along shift axis
        shift_array = np.zeros(
                shape=(shift_axis_len, len(shift_sigmas)),
                dtype=int)
        ...
        for shift_axis_position in range(shift_axis_len):
            # only shift at the center frame
            if shift_axis_position == 3:
                r = random.random()
                slip = np.array([np.random.normal(scale=sigma
                                 / lcm_voxel_size[dim])
                                 for dim, sigma in enumerate(shift_sigmas)])
        ...
        center_shift = shift_array[3]
        shift_array -= center_shift
        return shift_array

    @staticmethod
    def compute_upstream_roi(request_roi, sub_shift_array):
        """ Compute the ROI to pass upstream for a specific item
            (array or points) in a request

        :param request_roi: the downstream ROI passed to the Jitter node
        :param sub_shift_array: the portion of the global shift array that
            should be used to shift the item
