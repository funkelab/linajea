"""Provides an adapted version of the gunpowder ElasticAugment node
"""
import logging
import math
import random
...
class ElasticAugment(BatchFilter):
    ...
    Args:
        ...
        rotation_3d (``bool``):
            ...
            Do 3d rotations (instead of just 2d, has to be supported by the
            augment library)?
        ...
        temporal_dim (``bool``):
            ...
            If the data contains a temporal dimension. If true it is assumed to
            be the one before the spatial dimensions: ``(b, c, t, z, y, x)``
    ...
    def prepare(self, request):
        ...
        for key, spec in request.items():
            ...
            # remember target ROI (this is where the transformation will
            # project to)
            ...
            if self.temporal_dim:
                ...
                # make sure the target ROI aligns with the voxel grid (which
                # might not be the case for points)
                target_roi_temporal = target_roi_temporal.snap_to_grid(
                    (1,) + tuple(self.voxel_size), mode="grow")
                logger.debug(
                    "downstream request spatial ROI aligned with voxel grid "
                    "for %s is %s",
                    key,
                    target_roi_temporal,
                )

                # remember target ROI incl potential temporal dim
                # (this is where the transformation will project to)
                self.target_rois_temporal[key] = target_roi_temporal
    ...
    def process(self, batch, request):
        ...
        for (array_key, array) in batch.arrays.items():
            ...
            logger.debug("array in shape %s", data.shape)
            logger.debug("%s %s",
                         array_key, self.spec[array_key].interpolatable)
            data = np.array(
                [
        ...
        for (graph_key, graph) in batch.graphs.items():
            ...
            logger.debug("num points begin %s", len(nodes))
            if self.use_fast_points_transform:
                missed_nodes = self.__fast_point_projection(
                    self.transformations[graph_key],
                    nodes,
                    graph.spec.roi,
                    self.target_rois[graph_key] if not self.temporal_dim
                    else self.target_rois_temporal[graph_key],
                    graph_key, request,
                )
            ...
            for node in missed_nodes:
                ...
                location_voxels = (location[-self.spatial_dims:] /
                                   self.voxel_size)
            ...
            logger.debug("num points end %s", len(list(graph.nodes)))
            # restore original ROIs
        ...
        for array_key in request.array_specs.keys():
            ...
            if voxel_size is None:
                ...
                assert voxel_size == vs, \
                        "ElasticAugment can only be used with arrays of " \
                        "same voxel sizes, but %s has %s, and %s has %s." % (
    ...
    def __create_transformation(self, target_shape):
        ...
        if self.rotation_3d:
            rotation = [random.random() * self.rotation_max_amount +
                        self.rotation_start,
                        random.random() * self.rotation_max_amount +
                        self.rotation_start,
                        random.random() * self.rotation_max_amount +
                        self.rotation_start]
            do_rotation = any(rotation)
        else:
            rotation = (random.random() * self.rotation_max_amount +
                        self.rotation_start)
            do_rotation = rotation != 0
    ...
    def __fast_point_projection(
        self, transformation, nodes, source_roi, target_roi, key, request
    ):
        ...
        with warnings.catch_warnings():
            ...
            projected_locs = ndimage.measurements.center_of_mass(
                data > 0, data, ids)
        dims = (self.spatial_dims if not self.temporal_dim
        ...
        for point_id, proj_loc in zip(ids, projected_locs):
            ...
            if not any([np.isnan(x) for x in proj_loc]):
                assert len(proj_loc) == dims, (
                    "projected location has wrong number of dimensions: {}, "
        ...
        logger.debug(
            "{} of {} {} points lost in fast points projection of {}".format(
                len(missing_points), len(ids), len(nodes), key
            )
        )
    ...
    def __project(self, transformation, location):
        """Find the projection of location given by transformation. Returns
        None if projection lies outside of transformation."""
    ...
    def __misalign(self, transformation):
        ...
        dims = 3
        bb_min = tuple(int(math.floor(transformation[d].min()))
                       for d in range(dims))
        bb_max = tuple(int(math.ceil(transformation[d].max())) + 1
                       for d in range(dims))
        logger.debug("min/max of transformation: %s/%s", bb_min, bb_max)
        ...
        bb_max = tuple(int(math.ceil(transformation[d].max())) + 1
                       for d in range(dims))
        logger.debug("min/max of transformation after misalignment: %s/%s",
                     bb_min, bb_max)
    ...
    def __random_offset(self):
        ...
        return Coordinate(
            ...
            + tuple(
                self.max_misalign -
                random.randint(0, 2 * int(self.max_misalign))
                for d in range(2)
            )
        )
