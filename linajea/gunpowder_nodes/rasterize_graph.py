"""Provides an adapted version of the gunpowder RasterizeGraph node
"""
import logging
import numpy as np
...
class RasterizationSettings(Freezable):
    ...
    Args:
        ...
        mode (``string``):
            ...
            One of ``ball`` or ``peak``. If ``ball`` (the default), a ball with
            the given ``radius`` will be drawn. If ``peak``, the point will be
        ...
        color_attr (``str``, optional)
            ...
            - color_attr must be populated for nodes and edges upstream of this
              node
    '''
    def __init__(
            self,
        ...
        if inner_radius_fraction is not None:
            assert 0.0 < inner_radius_fraction < 1.0, (
...
class RasterizeGraph(BatchFilter):
    ...
    def setup(self):
        ...
        if self.array_spec.dtype is None:
            if self.settings.mode == 'ball' or self.settings.mode == "point":
    ...
    def process(self, batch, request):
        ...
        if graph.num_vertices == 0:
            ...
            if len(labels) == 0:
                logger.debug("Graph and provided object mask do not overlap."
                             "No graph to rasterize.")
                rasterized_graph_data = np.zeros(
                    data_roi.get_shape(), dtype=self.spec[self.array].dtype)
            else:
                ...
                rasterized_graph_data = np.sum(
                    [
                        self.__rasterize(
                            ...
                            self.settings,
                            Array(data=mask_array.data == label,
        ...
        if (self.settings.bg_value != 0 or
            ...
            rasterized_graph_data = replaced.astype(
                self.spec[self.array].dtype)

        # create array and crop it to requested roi
        spec = self.spec[self.array].copy()
        spec.roi = data_roi*voxel_size
        rasterized_graph = Array(
            data=rasterized_graph_data,
            spec=spec)
        batch[self.array] = rasterized_graph.crop(request[self.array].roi)

    def __rasterize(self, graph, data_roi, voxel_size, dtype, settings,
                    mask_array=None):
        ...
        for node in graph.nodes:
            ...
            if use_fast_rasterization:
                ...
                # Calculate where to crop the kernel mask and the rast. array
                ...
                if settings.color_attr is not None:
                    ...
                    assert c is not None, \
                        f"color_attr set but not in node {node}"
                    rasterized_graph[arr_crop_ind] = \
                        c * np.logical_and(
                            ball_kernel[kernel_crop_ind],
                            rasterized_graph[arr_crop_ind] == 0)
                else:
                ...
                if settings.color_attr is not None:
                    ...
                    if c is None:
                        ...
                        continue
                    elif (np.isclose(c, 1) and
                          not np.isclose(settings.fg_value, 1)):
                        logger.warning(
                            f"Node {node} is being colored with color {c} "
                            f"according to attribute {settings.color_attr} "
                            "but color 1 will be replaced with fg_value: "
                            f"{settings.fg_value}"
                            )
                else:
                    c = 1
                ...
                if settings.color_attr is not None:
                    ...
                    if c is None:
                        continue
                          ...
                          not np.isclose(settings.fg_value, 1)):
                        logger.warning(
                            f"Edge {e} is being colored with color {c} "
                            f"according to attribute {settings.color_attr} "
                            "but color 1 will be replaced with fg_value: "
                            f"{settings.fg_value}"
                            )
        ...
        if not use_fast_rasterization:
            ...
            if settings.mode == "ball":
                ...
                gaussian_filter(
                    rasterized_graph, sigmas, output=rasterized_graph,
                    mode="constant"
                )
            ...
            elif settings.mode == "point":
                # dont change, keep single point
                pass

        if mask_array is not None:
            # use more efficient bitwise operation when possible
            if settings.mode == 'ball' or self.settings.mode == "point":
