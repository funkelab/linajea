...
def solve_blockwise(**kwargs):
    ...
    for sample in samples:
        ...
        solve_blockwise_sample(sample=sample, **sample_kwargs)

def solve_blockwise_sample(**kwargs):
    if 'db_name' not in kwargs['general']:
        kwargs['general']['db_name'] = checkOrCreateDB(**kwargs)

    parameters = TrackingParameters(**kwargs['solve'])
    ...
    logger.info("from scratch %s", kwargs['solve']['from_scratch'])
    if kwargs['solve']['from_scratch']:
    ...
    success = daisy.run_blockwise(
        ...
        check_function=lambda b: check_function(
            ...
            kwargs['general']['db_name'],
            kwargs['general']['db_host']),
        num_workers=kwargs['solve']['num_workers'],
        fit='overhang')
    if success:
